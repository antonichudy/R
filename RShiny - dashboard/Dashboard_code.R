...
library(ncdf4)
library(raster)
#library(rgdal) # do not support with latest version of R
library(ggplot2)
library(chron)
library(lattice)
...
library(sp)
library(maps)
#library(maptools) # do not support with latest version of R
library(shiny)
library(tidyverse)
library(tidyr)
...
d_c <- death_cause %>% 
  filter(Entity == 'World') %>%
  filter(Year > 1950) %>% 
  dplyr::select(Year,Entity, deaths_drought, deaths_temperature, deaths_wildfire) %>% 
  dplyr::arrange(Year) %>% 
  pivot_longer(
...
air_pollution<- air_pollution %>% 
  ...
  dplyr::select(Entity, Percentage, Year)
plus<-country %>% 
...
air <- air %>%
  ...
  pivot_longer(
    ...
    values_drop_na = TRUE)
air <- air %>% 
  dplyr::select(Year, lp_zgon, przyczyna_smierci) %>% 
  arrange(Year)
...
malaria <- mala %>% 
  filter(Entity == 'World') %>% 
  dplyr::select(Year, deaths = Deaths...Malaria...Sex..Both...Age..Age.standardized..Rate.)
